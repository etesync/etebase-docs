...
etebase_item_manager_transaction(item_mgr, items, ETEBASE_UTILS_C_ARRAY_LEN(items), NULL, 0, NULL);
// Will succeed
etebase_item_manager_batch(item_mgr, items, ETEBASE_UTILS_C_ARRAY_LEN(items), NULL, 0, NULL);
// Will succeed because item2 hasn't changed on device B
const EtebaseItem *items2[] = { item2 };
etebase_item_manager_transaction(item_mgr, items2, ETEBASE_UTILS_C_ARRAY_LEN(items2), NULL, 0, NULL);
...
etebase_fetch_options_set_stoken(fetch_options, stoken);
const EtebaseItem *items[] = { item };
etebase_item_manager_transaction(item_mgr, items, ETEBASE_UTILS_C_ARRAY_LEN(items), NULL, 0, fetch_options);
etebase_item_manager_batch(item_mgr, items, ETEBASE_UTILS_C_ARRAY_LEN(items), NULL, 0, fetch_options);
...
etebase_item_manager_batch(item_mgr, items, ETEBASE_UTILS_C_ARRAY_LEN(items), deps, ETEBASE_UTILS_C_ARRAY_LEN(deps), NULL);
etebase_item_manager_transaction(item_mgr, items, ETEBASE_UTILS_C_ARRAY_LEN(items), deps, ETEBASE_UTILS_C_ARRAY_LEN(deps), NULL);
...
// Get the item out of the collection
Item colItem = collection.getItem();
...
etebase_item_manager_transaction(item_mgr, items, ETEBASE_UTILS_C_ARRAY_LEN(items),
...
// Or fetch updates of the collection along with other items:
ItemListResponse items = itemManager.fetchUpdates(new Item[] { collection.getItem(), item1 });
```
...
// Or fetch updates of the collection along with other items:
val items = itemManager.fetchUpdates(arrayOf(collection.getItem(), item1))
```
